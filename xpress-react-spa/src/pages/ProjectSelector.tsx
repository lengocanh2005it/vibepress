...
const ProjectSelector = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        if (email) {
          const data = await getRepoByEmail(email);
  ...
  useEffect(() => {
    if (selectedRepo) {
      const fetchData = async () => {
        try {
          const data = await getCommitHistory(selectedRepo.wpRepoUrl);
          setCommitHistory(data);
          console.log("Commit history for", selectedRepo.wpRepoName, data);
        } catch (error) {
    ...
    <div className="flex-1 w-full max-w-[1400px] mx-auto px-8 py-10 flex gap-8">
      ...
      <aside className="w-80 shrink-0 flex flex-col gap-6">
        <div className="flex items-center justify-between mb-2">
          <h2 className="font-headline text-[20px] font-bold text-[#1a2b21]">
            Connected Repositories
          </h2>
          <span className="material-symbols-outlined text-[#5c6860] cursor-pointer">
            link
          </span>
        </div>
        ...
        <div className="flex flex-col gap-4">
          {repositories.map((repo) => (
            <div
              key={repo.siteId}
              className="bg-[#e8e6df]/50 border-2 border-transparent hover:border-[#dcd9ce] rounded-3xl p-6 cursor-pointer transition-all"
              onClick={() => setSelectedRepo(repo)}
            >
              <div className="flex items-center gap-3 mb-2">
                <span className="material-symbols-outlined text-[#233227] text-[20px]">folder_zip</span>
                <h3 className="font-bold text-[#233227] text-[15px]">
                  {repo.siteName || repo.wpRepoName}
                </h3>
              </div>
              <p className="font-mono text-[11px] text-[#8e9892] mb-6">
                {repo.wpRepoUrl.replace('https://', '')}
              </p>
              <div className="flex justify-between items-center mt-auto">
                <span className="bg-[#c2e4cc] text-[#2c6e49] text-[10px] uppercase font-bold tracking-widest px-3 py-1 rounded-full">
                  Connected
                </span>
                <span className="text-[11px] text-[#8e9892]">
        ...
        <div className="mt-auto bg-[#594d3f] rounded-[2rem] p-8 text-[#f4ead5] relative overflow-hidden shadow-xl">
          <h4 className="text-[13px] font-medium opacity-80 mb-2">
            Total Assets Hosted
          </h4>
          <p className="font-headline text-[38px] font-bold leading-none">
            1.2 GB
          </p>
          <span className="material-symbols-outlined text-[120px] absolute -bottom-6 -right-6 opacity-10 rotate-12">
            cloud_done
          </span>
        </div>
      </aside>
      ...
      <main className="flex-1 flex flex-col gap-6">
        ...
        <div className="bg-[#FAF7F0] rounded-[2.5rem] p-8 border border-[#e8e6df] shadow-[0_8px_30px_rgba(0,0,0,0.02)]">
          <div className="flex justify-between items-end mb-8">
            <div>
              <h2 className="font-headline text-[22px] font-bold text-[#1a2b21] mb-1">
                Page Manager
              </h2>
          ...
          <div className="mt-6 text-center">
            <button className="text-[13px] font-bold text-[#233227] hover:text-[#49704F] transition-colors">
              View All 24 Pages
            </button>
          </div>
        </div>
        ...
        <div className="bg-[#FAF7F0] rounded-[2.5rem] p-8 border border-[#e8e6df] shadow-[0_8px_30px_rgba(0,0,0,0.02)]">
          <div className="flex justify-between items-start mb-8">
            ...
            <span className="material-symbols-outlined text-[#8e9892]">
              history
            </span>
          </div>
                ...
                <div key={commit.sha} className="flex gap-4">
                  ...
                  <div className="pt-1">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <span className="font-mono text-[10px] bg-[#e8e6df] text-[#5c6860] px-2 py-0.5 rounded border border-[#dcd9ce]">{commit.sha}</span>
                    </div>
