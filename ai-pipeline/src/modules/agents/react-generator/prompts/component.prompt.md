...
- If a node includes `customClassNames`, preserve those classes on the equivalent JSX element:
  ...
  - **CRITICAL for `vp-hover-shadow` buttons**: the CSS hover rule sets `background-color: blue`. This is overridden by any `style={{ backgroundColor: "..." }}` inline prop because inline styles have higher specificity than class-based CSS. **Never use `style={{ backgroundColor }}` together with `vp-hover-shadow`**. Instead, use a Tailwind `bg-[#hex]` class for the base color so the CSS `:hover` rule can take effect.
