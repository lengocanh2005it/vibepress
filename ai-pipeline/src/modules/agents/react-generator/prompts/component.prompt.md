...
- HTML from API:
  - post/body and page/body HTML must render through `renderRichTextChildren(...)` or equivalent structured JSX that parses the HTML and recursively maps each node to concrete React tags (`p`, `h1`-`h6`, `ul`, `li`, `a`, `img`, `strong`, `em`, etc.)
  - ⛔ NEVER render `.content` fields with `dangerouslySetInnerHTML`, including `post.content`, `page.content`, and `item.content`
  - ⛔ NEVER use `innerHTML` APIs for page/detail body content; render the parsed node tree instead
...
| `post-content` / `html` | render approved body HTML through `renderRichTextChildren(...)` or equivalent structured JSX that walks the HTML node tree and emits concrete React tags; never render `.content` fields with `dangerouslySetInnerHTML` or any `innerHTML` API                                                                                                                                                                                            |
